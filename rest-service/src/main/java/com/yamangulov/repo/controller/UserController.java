...
public class UserController {
    ...
    @PostMapping("/addSkill/{user_id}/{skill_id}")
    @ApiOperation("Добавление компетенции пользователя")
    public String addSkill(@PathVariable UUID user_id, @PathVariable Integer skill_id) {
        return userService.addSkill(user_id, skill_id);
    }

    @PostMapping("/removeSkill/{user_id}/{skill_id}")
    @ApiOperation("Удаление компетенции пользователя")
    public String removeSkill(@PathVariable UUID user_id, @PathVariable Integer skill_id) {
        return userService.removeSkill(user_id, skill_id);
    }
}
